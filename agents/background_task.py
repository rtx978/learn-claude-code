...
class BackgroundManager:
    def __init__(self):
        ...
        self._notification_queue = NotificationQueue() # completed task results
        self._lock = threading.Lock()
    ...
    def _execute(self, task_id: str, command: str):
        ...
        # 如果任务超时或报错，提升为高优先级，让大模型第一眼看到
        priority = "high" if status in ("error", "timeout") else "medium"
        # 传入 key=task_id，如果后台同一个任务疯狂发通知，旧的会被自动替换！
        self._notification_queue.push(message=notif_data, priority=priority, key=task_id)
    ...
    def drain_notifications(self) -> list:
        ...
        # 🌟 修改点 3：直接委托给高级队列的 drain 方法，无需自己再加锁和清空
