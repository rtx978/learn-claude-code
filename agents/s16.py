...
# -- Task board scanning --
def _append_claim_event(payload: dict):# 记录“抢单流水”的审计日志
    TASKS_DIR.mkdir(parents=True, exist_ok=True)
    with CLAIM_EVENTS_PATH.open("a", encoding="utf-8") as f:
        f.write(json.dumps(payload) + "\n")
def _task_allows_role(task: dict, role: str | None) -> bool:  #权限校验，判断当前的 Agent 角色是否“配得上”这个任务。
    required_role = task.get("claim_role") or task.get("required_role") or ""
    if not required_role:
        return True
    return bool(role) and role == required_role
def is_claimable_task(task: dict, role: str | None = None) -> bool: #终极漏斗，判断一个任务当前到底能不能被“接盘”
    return (
